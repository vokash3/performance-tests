...
class LocustInterceptor(UnaryUnaryClientInterceptor):
    ...
    def intercept_unary_unary(self, continuation, client_call_details, request):
        ...
        UnaryUnaryClientInterceptor — это базовый класс из библиотеки grpc,
        который позволяет внедрить промежуточную логику между gRPC клиентом
        и сервером для обычных (нестриминговых) запросов.
        continuation(client_call_details, request) — фактический вызов gRPC-метода. Мы просто оборачиваем его,
        чтобы засечь время, перехватить исключения и отправить метрику.
        start_time = time.perf_counter() и далее вычисление response_time.
        response.result().ByteSize() — получаем размер тела ответа (необходимо вызвать .result() у future-объекта).
        self.environment.events.request.fire(...) — основная точка, через которую Locust собирает статистику.
